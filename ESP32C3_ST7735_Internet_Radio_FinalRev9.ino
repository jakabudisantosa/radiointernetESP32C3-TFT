/*
  ESP32-C3 SUPERMINI INTERNET RADIO
  ...
  BUTTON RADIO/VOLUME:
    GPIO20 -> PUSH BUTTON -> GND

  BUTTON MODE:
    GPIO21 -> PUSH BUTTON -> GND
...
// BACKLIGHT: RADIO = selalu ON, NTP = ON 10 menit setelah aktivitas/tombol
...
const char* DEFAULT_RADIO_NAMES[] = {
  ...
  // =========================================================
  // 🌍 INTERNASIONAL / ASAL TIDAK CUKUP JELAS
  // =========================================================
  "Nightwave Plaza",
...
const char* DEFAULT_RADIO_URLS[] = {
  ...
  "http://shoutcast.wrn.org:8025/nhk41left",
  "http://ottava2.out.airtime.pro:8000/ottava2_a",
  // =========================================================
  // 🇰🇷 KOREA SELATAN
  // =========================================================
  ...
  "http://radio.freeplace.info:8000/RadioHoro.mp3",
...
void initRadioDatabase() {
  ...
  if (!loadRadioDatabase()) {
    Serial.println("[RADIO DB] /radios.json belum ada -> membuat dari data Rev3");
...
// Mengikuti engine streaming sketch 1
#define AUDIO_BUFFER_SIZE (256 * 1024)
const unsigned long BUFFER_WARMUP_TIME = 1500;
...
void startRadio(bool isReconnect = false) {
  ...
  Serial.println("[1] ICY stream");
  stream = new AudioFileSourceICYStream(stations[currentStation].url.c_str());
  if (!stream) {
    Serial.println("[ERROR] ICY stream allocation FAILED");
    stopStream();
    return;
  }

  // PENTING: constructor ICYStream dapat dibuat walaupun HTTP request gagal.
  // Jangan teruskan ke decoder jika stream sebenarnya tidak terbuka.
  if (!stream->isOpen()) {
    Serial.println("[ERROR] STREAM TIDAK TERBUKA - URL kemungkinan tidak valid / server tidak merespons");
    stopStream();
    return;
  }

  Serial.println("[OK] ICY stream terbuka");
  Serial.println("[2] Buffer 256 KB");
  ...
  Serial.println("[3] MP3 decoder");
  mp3 = new AudioGeneratorMP3();
  if (!mp3) {
    Serial.println("[ERROR] MP3 allocation FAILED");
    stopStream();
    return;
  }

  Serial.println("[4] MP3 begin");
  ...
  Serial.println("[RADIO] BUFFER WARM-UP 1500 ms");
...
void drawRadioScreen() {
  drawHeader("ESP32-C3 INTERNET RADIO");
...
String radioManagerHTML() {
  ...
  html += F("<meta charset='utf-8'><title>ESP32 Radio Manager Rev6</title><style>");
  ...
  html += F("<div class='card'><h2>ESP32-C3 INTERNET RADIO</h2>");
  html += F("<div class='sub'>Radio Manager Rev6 &mdash; 10 radio per halaman</div>");
...
void handleRadioAdd() {
  ...
  if (stationCount >= MAX_STATIONS) {
    server.send(400, "text/plain", "Database penuh (maksimum 100 radio)");
    return;
  }
...
void setup() {
  ...
  Serial.println();
  Serial.println("================================");
  Serial.println("ESP32-C3 INTERNET RADIO");
